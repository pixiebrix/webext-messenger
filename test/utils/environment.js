...
const getZippedExtensionAsPath = onetime(async () => {
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'pixie-'));
  const temporaryZippedFile = path.join(temporaryDirectory, 'extension.zip');
  console.log('Storing zip in', temporaryZippedFile);
  await fs.promises.writeFile(
    temporaryZippedFile,
    await getZippedExtensionAsBuffer(),
  );
  return temporaryZippedFile;
});

const getZippedExtensionAsBuffer = onetime(async () => {
...
async function runInChrome() {
  ...
  builder.withCapabilities({
    browser: 'chrome',
    browserName: 'chrome',
    browser_version: 'latest', // eslint-disable-line camelcase
    'bstack:options': {
      os: 'Windows',
      osVersion: '10',
    },
